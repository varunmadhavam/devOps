...
if [ -f $DEPLOY_DIRECTORY/dodeploy ]; then
   ...
   echo -n `date +"%b %d %T"`" "`hostname`" $APPID: ";rm -v --interactive=never $DEPLOY_DIRECTORY/dodeploy 2>&1>>$LOG
   if [ $? -ne 0 ];then
 ...
 do
  ...
  if [ -f $TARGET_DIRECTORY/$file ]; then
     ...
     if [ -f $BACKUP/$file.bak ]; then
      ...
      echo -n `date +"%b %d %T"`" "`hostname`" $APPID: ";cp -pv $BACKUP/$file.bak $BACKUP/$file.$(date +"%d%b%T")>>$LOG
      echo -n `date +"%b %d %T"`" "`hostname`" $APPID: ";cp -pv $TARGET_DIRECTORY/$file $BACKUP/$file.bak>>$LOG
     else
      echo -n `date +"%b %d %T"`" "`hostname`" $APPID: ";cp -pv $TARGET_DIRECTORY/$file $BACKUP/$file.bak>>$LOG
     fi
     if [ -f $BACKUP/$file.bak ]; then
      ...
      echo -n `date +"%b %d %T"`" "`hostname`" $APPID: ";cp -pv $DEPLOY_DIRECTORY/$file $TARGET_DIRECTORY>>$LOG
      if [ -f $TARGET_DIRECTORY/$file ];then
...
do
   [[ "${LOGLINE}" == *"INFO: Server startup in"* ]]  && pkill -P $$ timeout
done
x=$?
if [ $x -eq 1 ]; then
